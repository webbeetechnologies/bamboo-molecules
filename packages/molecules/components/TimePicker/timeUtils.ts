...
export function getHourTypeFromOffset(left: number, top: number, size: number): PossibleHourTypes {
    const w = size / 2;
    const x = w - left;
    const y = size / 2 - top;
...
export function getHours(handAngle: number, hourType: PossibleHourTypes | undefined) {
