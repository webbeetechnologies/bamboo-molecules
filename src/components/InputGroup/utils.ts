...
export const getAttachedChildren = (children: any) => {
    ...
    */
    const trailingChildrenWithSpacing = trailingChildren.map((child: any) => {
        return cloneElement(child, marginProp, child.props.children);
    });
    /*
    | New children array with applied margin to trailing children
    */
    return [
        cloneElement(childrenArray[0], leftElemProp, childrenArray[0].props.children),
...
export const inputGroupStyles: ComponentStylePropWithVariants<TextStyle> = {
    flexDirection: 'row',
};
