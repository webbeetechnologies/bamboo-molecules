...
import { Sortable, ESortableActions, ESortDirection, NotSortableReturnProps, OnSort, OnSortAction, Sort, SortableDataSource, SortableDataSourceProps, SortableDataSourcePropsEnabled } from './types';
...
const defaultOnSort: OnSort = (dataSource, args) => {
    ...
    switch (args.type) {
        ...
        case ESortableActions.ReorderSort:
            if (args.payload.prevIndex > sort.length - 1) throw new Error(`Item doesn't exist at position ${args.payload.prevIndex}`);
            sort = [ ...sort ];
...
const applySortOnRecords = <T extends {}>(datasource: SortableDataSourcePropsEnabled<T>) => {
    ...
    let predicate = zip(...sort?.map(({ column, direction }) => [column, direction === ESortDirection.Desc ? "desc" : "asc"]));
...
export const useSortableDataSource = <T extends {}>(
    ...
    const {records, isSortable, onSort = defaultOnSort, sorting: { isNestedSort, disabled, sort }, ...rest} = sortableDatasourceProps;
